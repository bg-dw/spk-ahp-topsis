        ...
        if (count($id_lokasi) >= 2) {
            if ($nilai) { //jika nilai kosong
                ...
                for ($i = 0; $i < count($id_alt); $i++) {
                    for ($j = 0; $j < count($id); $j++) {
                        $m_normalisasi[$i][$j] = (($nilai_alt[$i][$j]) / ($pembagi_ts[$j]));
                        // echo $nilai_alt[$i][$j] . " / " . ($pembagi_ts[$j]) . "   |   ";
                    }
                    // echo "<br>";
                }
                ...
                for ($i = 0; $i < count($id_alt); $i++) {
                    for ($j = 0; $j < count($id); $j++) {
                        $m_terbobot[$i][$j] = (($m_normalisasi[$i][$j]) * ($bbt[$j]));
                    }
                }
                // var_dump($m_normalisasi);
                ...
                $data['content'] = 'guest/V_hasil_perhitungan';
                $data['id_alt'] = $id_alt;
                $data['alt'] = $nama_alt;
                ...
                redirect(base_url('c_perhitungan/'));
            }
        } else {
